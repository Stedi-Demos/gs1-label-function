...
export const handler = async (labelJSON: typeof labelJSONSample) => {
  ...
  const { value: serialNumber } = await stash.send(
    new IncrementValueCommand({
      keyspaceName: requiredEnvVar("STASH_KEYPSPACE_NAME_GS1"),
  ...
  const poNumber = headerText(`PO: ${labelJSON.purchaseOrder.number}`);
  poNumber.top = 20;
  row3LeftBox.content.push(poNumber);

  const quantity = baseText(`QTY: ${labelJSON.purchaseOrder.quantity}`);
  quantity.top = 140;
  ...
  const style = baseText(`STYLE: ${labelJSON.purchaseOrder.cartons.style}`);
  style.top = 70;
  row3RightBox.content.push(style);

  const color = baseText(`COLOR: ${labelJSON.purchaseOrder.cartons.color}`);
  color.top = 110;
  row3RightBox.content.push(color);

  const size = baseText(`SIZE: ${labelJSON.purchaseOrder.cartons.size}`);
  size.top = 150;
    ...
    await Promise.all([
      buckets.send(
        new PutObjectCommand({
          bucketName: requiredEnvVar("LABEL_BUCKET_NAME"),
          key: pdfPath,
          body: buffer,
        })
      ),
